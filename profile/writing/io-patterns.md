...
frontmatter 适合保存文档级别的稳定状态，例如 `audience`、`authors`、`reviewed_by`、`purpose`、`updated`。这些信息应该可以被 agent 稳定读取，而不是散落在正文描述里。
...
- prompt SHOULD 保持为任务指令，不要承担长期配置数据库的职责。
