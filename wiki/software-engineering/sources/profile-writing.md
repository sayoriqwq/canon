...
reviewed_by:
  - sayori
purpose: 消化 wiki/profile/writing 中的写作偏好，并连接到 software-engineering 理论资产。
type: source
status: active
confidence: high
sources: []
...
本页把 `wiki/profile/writing/` 作为内部 source 消化。`wiki/profile/writing/` 仍然是当前写作协作规则的权威位置；本页和下游 software-engineering assets 负责把这些规则提升成可连接、可修订的理论资产。

本 source note 不复制 profile 原文，只提炼它对理论系统的贡献。

## 核心贡献

`wiki/profile/writing/` 提供了一套关于 agent-facing 文档的理论雏形：
